...
calc_log_likelihood <- function(obs_data, pred_dist) {
  ll <- 0
  epsilon <- 1e-2 #1e^-2 used for the epsilon parameter, as it best matches the smallest possibility this dataset can distinguish from zero (1/270).
...
grid <- expand.grid(
  pA = seq(0.005, 0.02, 0.005),   
  pB = seq(0.002, 0.006, 0.001),    
  w  = seq(0.006, 0.01, 0.001),   
  peakA  = c(45.5),              
  peakB  = c(92),
  sigmaA_gain = c(1.5, 2, 2.5),                  
  sigmaA_loss = c(1.5, 2 ,2.5),                  
  sigmaB_gain = seq(0.5, 0.7, 0.05),                 
  sigmaB_loss = seq(0.5, 0.7, 0.05),                 
...
ggplot() +
  ...
  xlim(20, 125) +
  labs(
    title = "Markov Chain Model Fit vs. Observed Prostrate Adenocarcinoma Data",
